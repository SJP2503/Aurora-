...
# Normalizes a raw SOCE CSV you drop at data/input/comelec_soce_raw.csv
normalize_soce <- function(path = "data/input/comelec_soce_raw.csv") {
  ...
  recip <- nm[which(nm %in% c("recipient","candidate","candidate_party","beneficiary"))[1]]
  amt   <- nm[which(nm %in% c("amount","amount_donated","value","amount_php"))[1]]
  year  <- nm[which(nm %in% c("year","filed_year","election_year"))[1]]
  ref   <- nm[which(nm %in% c("ref","reference","doc","source","url"))[1]]
